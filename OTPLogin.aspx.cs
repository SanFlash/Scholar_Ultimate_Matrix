...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            // Invalid credentials
            Response.Write("<script>alert('Invalid email, password, or user role is not Admin');</script>");
        }
    }
    ...
    private bool IsValidUser(string email, string password)
    {
        ...
        {
            string query = "SELECT COUNT(1) FROM SignUp WHERE email = @Email AND password = @Password AND [user] = 'Admin'";
